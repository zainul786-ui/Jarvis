...
export const YouTubePlayer: React.FC<{ videoId: string; title: string; onClose: () => void }> = ({ videoId, title, onClose }) => {
    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                className={`fixed bottom-32 right-8 z-50 overflow-hidden rounded-xl border border-cyan-500/30 bg-[#030814]/90 backdrop-blur-md shadow-2xl transition-all duration-300 ${isMinimized ? "w-64 h-16" : "w-[400px] h-[300px]"}`}
            >
                <div className="flex items-center justify-between bg-cyan-500/10 px-4 py-2 border-b border-cyan-500/20">
                    ...
                    <div className="flex items-center space-x-1">
                        <button onClick={() => setIsMinimized(!isMinimized)} className="p-1 hover:bg-white/10 rounded-md text-cyan-400">
                            {isMinimized ? <Maximize2 className="w-4 h-4" /> : <Minimize2 className="w-4 h-4" />}
                        </button>
                        <button onClick={onClose} className="p-1 hover:bg-red-500/20 rounded-md text-red-400">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                </div>

                <div className={`relative bg-black ${isMinimized ? "hidden" : "block"}`}>
                    {!isReady && (
